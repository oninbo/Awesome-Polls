...
object AwesomePollsBot {
  ...
  def poll[F[_]: TelegramClient: Timer](
      users: Ref[F, Map[Long, UserState]]
  )(implicit sendMessages: Boolean): Scenario[F, Unit] =
    for {
  ...
  def onMessage[F[_]: TelegramClient: Timer](
      users: Ref[F, Map[Long, UserState]]
  )(implicit sendMessages: Boolean): Scenario[F, Unit] =
    for {
            ...
            case Some(CreatePoll(question, options)) =>
              for {
                _ <- question match {
                      case Some(value) => {
                        Scenario.eval(
                          users.update(_ + (message.chat.id -> CreatePoll(Some(value), options :+ message.text)))
                        )
                      }
                      case None => {
  ...
  def done[F[_]: TelegramClient: Timer](
      users: Ref[F, Map[Long, UserState]]
  )(implicit sendMessages: Boolean): Scenario[F, Unit] =
    for {
      ...
      _ <- usersMap.get(message.chat.id) match {
            case Some(CreatePoll(question, options)) => {
              question match {
                case Some(value) => {
                  if (options.length < 2)
                    if (sendMessages) Scenario.eval(message.chat.send("Not enough options.")) else Scenario.done[F]
                  else
                    for {
