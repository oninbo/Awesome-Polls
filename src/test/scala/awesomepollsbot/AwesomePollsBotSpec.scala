...
class AwesomePollsBotSpec extends AnyFlatSpec with MockFactory with Matchers {
  ...
  implicit val tgClient: TelegramClient[IO] =
    new TelegramClient[IO] {
      override def execute[Req, Res](request: Req)(implicit M: Method[Req, Res]): IO[Res] = fail(
        "Telegram requests should not be called."
      )
    }
  ...
  it should "not remove user if there are not enough options" in {
    ...
    val messages: Stream[IO, TelegramMessage] = Stream(userMessage)
    testScenario(
      AwesomePollsBot.done,
      messages,
      _.get(userMessage.chat.id) should matchPattern {
  ...
  def testScenario(
      sc: Ref[IO, Map[Long, UserState]] => Scenario[IO, Unit],
      messages: Stream[IO, TelegramMessage],
      check: Map[Long, UserState] => Assertion,
      users: Map[Long, UserState]
  ): Unit = {
